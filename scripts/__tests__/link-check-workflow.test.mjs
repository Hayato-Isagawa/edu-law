...
test("この検査自身が PR の CI に配線されている", () => {
  ...
  // 「そもそも CI で走るか」は誰も見ていない。build.yml からステップを 1 行消すと、
  ...
  // 配線の検査を移して塞いだ。このリポでは `test:content` が別の口として走るので、
  // **このステップの削除はあちらが見ている**(`scripts/__tests__/content/` 側の
  // 「build.yml が test:workflows を〜呼ぶ」)。ここに残しているのは、`if:` の改変など
  // 削除以外の改変を、この口の主題と同じ場所で読めるようにするため。
  // 下の `test:content` の配線検査はその逆向き。
  //
...
test("oxlint と oxfmt の口が build.yml に配線されている", () => {
  // oxlint は warning でも exit 0、oxfmt --check は差分があれば exit 1。どちらも
  // ワークフローから外れると黙って検査が消える(ADR 0028)。
  const b = fs.readFileSync(
    path.join(HERE, "..", "..", ".github", "workflows", "build.yml"),
    "utf8"
  );
  for (const script of ["lint", "format:check"]) {
    assert.match(
      ...
      `build.yml に ${script} が無い、または前段が落ちると走らない形になっている`
    );
    // continue-on-error が付くと赤が job に伝わらない(通知ステップと同じ穴)。
    const step = b
      .split(/^ {6}(?=- name: )/m)
...
test("書名一致ガードの回帰テストが PR の CI に配線されている", () => {
  // 配線の検査は、**守る対象と違う口**に置かないと自分を守れない。
  // `scripts/__tests__/content/` 側にも同じ検査はあるが、`test:content` の
  // ステップごと消されるとあちらは実行されない。こちらは `test:workflows` の口で
  // 走るので、その削除を捕まえられる。1 つ上のテストが自分について書いている
  // 限界の、片側だけを埋める形。
  const b = fs.readFileSync(
    path.join(HERE, "..", "..", ".github", "workflows", "build.yml"),
    "utf8"
