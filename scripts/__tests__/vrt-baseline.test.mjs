...
test("ベースラインが PR の素材を --delete 付きで運んでいる", () => {
  ...
  for (const dir of CARRIED) {
    assert.match(
      b,
      new RegExp(`^ +rsync -a --delete ${dir}/ /tmp/edu-law-main/${dir}/$`, "m"),
      `${dir} を運んでいない、または --delete が落ちている`,
    );
  }
  // スキーマを同伴させないと、PR のフロントマターを main の zod が弾く。
  assert.match(b, /^ +cp src\/content\.config\.ts \/tmp\/edu-law-main\/src\/content\.config\.ts$/m);
});
...
test("素材を運ぶのがベースラインのビルドより前である", () => {
  ...
  const build = afterRawBranch.indexOf("npm --prefix /tmp/edu-law-main run build");
...
test("degraded 側の撮り直しは失敗を握りつぶさない", () => {
  ...
  assert.match(fallback, /npm --prefix \/tmp\/edu-law-main run build/, "撮り直しが無い");
...
test("degraded の復元が Astro のキャッシュまで消している", () => {
  ...
  for (const cache of ["/tmp/edu-law-main/.astro", "/tmp/edu-law-main/node_modules/.astro"]) {
