// VRT のベースラインを「main のコード x PR のコンテンツ」で撮る配線を固定する（ADR 0027）。
//
...
test("ベースラインが PR の素材を --delete 付きで運んでいる", () => {
  ...
  for (const dir of CARRIED) {
    assert.match(
      b,
      new RegExp(
        `^ +rsync -a --delete ${dir}/ /tmp/edu-law-main/${dir}/$`,
        "m"
      ),
  ...
  assert.match(
    b,
    /^ +cp src\/content\.config\.ts \/tmp\/edu-law-main\/src\/content\.config\.ts$/m
  );
  // **ファイルと worktree を動かす行は、この列挙と完全一致させる。** 上の 3 つが在る
  // ことしか見ないと、`rsync -a --delete src/ /tmp/edu-law-main/src/` や
  // `git -C /tmp/edu-law-main checkout "$GITHUB_SHA" -- src` を 1 行足して描画コード
  ...
  assert.deepEqual(carries, [
    ...
    "mv /tmp/edu-law-main/dist dist-main", // raw(neutral=false)の早期 exit
    ...
    "rm -rf /tmp/edu-law-main/dist /tmp/edu-law-main/.astro /tmp/edu-law-main/node_modules/.astro /tmp/edu-law-main/node_modules/.vite",
...
test("degraded への分岐がビルドコマンドだけに掛かっている", () => {
  ...
  assert.deepEqual(tolerated, ["npm --prefix /tmp/edu-law-main run build"]);
});
...
test("degraded の復元が Astro のキャッシュまで消している", () => {
  ...
  for (const cache of [
    "/tmp/edu-law-main/.astro",
    "/tmp/edu-law-main/node_modules/.astro",
  ]) {
    assert.ok(fallback.includes(cache), `${cache} を消していない`);
  }
  assert.match(fallback, /git -C \/tmp\/edu-law-main checkout -- \./);
  assert.match(fallback, /git -C \/tmp\/edu-law-main clean -fd/);
});
