...
// `fullPage` を落とす / 比較設定を緩める / 断面(viewport・`colorScheme`)を潰す / `use` で
// テーマを立てる JS を止める / 比較そのものを消す(`ignoreSnapshots`・`updateSnapshots`・
// `webServer` を main の dist に固定する)/ ワークフローの撮影コマンドに CLI フラグを足す・
// ステップを skip する — いずれも撮影件数を減らさないので `--list` からは見えない。
//
...
test("比較設定が VRT ジョブの環境でも同じ値になる", async () => {
  ...
  for (const [dist, config] of [
    ...
    assert.equal(config.webServer.command, `npx serve ${dist} -l 4175`);
  }
});
...
test("撮影の断面とリトライが固定されている", () => {
  ...
  // あれば赤になる形にする。
  assert.deepEqual(vrtConfig.use, { baseURL: "http://localhost:4175" });
...
test("test:content の口にあるテストファイルが 1 本である", () => {
  ...
  // ここで赤になる。
  const files = fs
    .readdirSync(path.join(ROOT, "scripts/__tests__/content"), {
      withFileTypes: true,
    })
