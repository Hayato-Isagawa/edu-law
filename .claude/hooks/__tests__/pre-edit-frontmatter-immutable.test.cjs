...
const KEY_CASES = [
  ...
  {
    key: "lastVerified",
    before: 'lastVerified: "2026-05-28"',
    after: 'lastVerified: "2026-08-09"',
  },
  {
    key: "publishedAt",
    before: '    publishedAt: "2022-12-06"',
    after: '    publishedAt: "2022-12-16"',
  },
  {
    key: "retrievedAt",
    before: '    retrievedAt: "2026-05-28"',
    after: '    retrievedAt: "2026-08-09"',
  },
];

test("KEY_CASES: 期待キーが網羅されている", () => {
  assert.deepEqual(
    KEY_CASES.map((c) => c.key),
    EXPECTED_KEYS
  );
});
...
test("フェンス無し: lastVerified の 1 行 Edit で発火する", () => {
  const out = run(
    edit('lastVerified: "2026-05-28"', 'lastVerified: "2026-08-09"')
  );
...
test("フェンス込みチャンクでも本文リンクの差し替えで発火する", () => {
  ...
  const out = run(
    edit(
      body("https://www.mext.go.jp/a.pdf"),
      body("https://www.mext.go.jp/b.pdf")
    )
  );
  assert.match(reasonOf(out), /^  urls \(inspected chunk\):$/m);
});
...
test("URL の切り出し: markdown リンクの閉じ括弧・引用符で止まる", () => {
  const m = captureProtectedFields(
    '[提要](https://a.go.jp/1)。 <https://b.go.jp/2> "https://c.go.jp/3"'
  );
...
test("URL の切り出し: 直前に文字が続くものは URL として扱わない", () => {
  assert.equal(
    captureProtectedFields("xhttps://a.go.jp/1").get("__urls__"),
    undefined
  );
});
...
test("URL の切り出し: スキームだけの文字列は拾わない", () => {
  assert.equal(
    captureProtectedFields("https:// と書いただけ").get("__urls__"),
    undefined
  );
});
...
test("本文のみの編集では発火しない", () => {
  const out = run(
    edit("学校現場でよく参照される。", "学校現場でとくによく参照される。")
  );
  assert.equal(out.exitCode, 0);
  assert.ok(!out.stdout);
});
...
test("引用符の有無だけの変更では発火しない", () => {
  assert.ok(
    !run(edit('lastVerified: "2026-05-28"', "lastVerified: '2026-05-28'"))
      .stdout
  );
});
...
test("保護フィールドの純粋な削除・挿入でも発火する", () => {
  assert.match(
    reasonOf(run(edit('lastVerified: "2026-05-28"', ""))),
    /^  lastVerified:$/m
  );
  assert.match(
    reasonOf(run(edit("", 'lastVerified: "2026-08-09"'))),
    /^  lastVerified:$/m
  );
});
...
test("LAW_PATH_RE: laws 配下にマッチする", () => {
  ...
  assert.match(
    "/Users/H/edu-law/src/content/laws/school-education-act.md",
    LAW_PATH_RE
  );
...
test("LAW_PATH_RE: laws 以外は対象外", () => {
  assert.doesNotMatch(
    "docs/decisions/0023-visual-regression-testing.md",
    LAW_PATH_RE
  );
...
test("対象外パスなら URL も e-Gov ID も見ない", () => {
  assert.ok(
    !run(edit("https://a.go.jp/1", "https://a.go.jp/2", "docs/x.md")).stdout
  );
  assert.ok(
    !run(edit("322AC0000000026", "322AC0000000027", "README.md")).stdout
  );
  assert.ok(
    !run(
      edit("322AC0000000026", "322AC0000000027", "src/components/Logo.astro")
    ).stdout
  );
});
...
const REAL_LAW = path.join(
  __dirname,
  "..",
  "..",
  "..",
  "src/content/laws/basic-act-on-education.md"
);
...
test("Write: 新規ファイルの作成は通す(比較対象が無い)", () => {
  const out = run(
    write("src/content/laws/does-not-exist-yet.md", "---\ntitle: 新法\n---\n")
  );
  assert.ok(!out.stdout);
});
...
test("理由文にファイルパスが入る", () => {
  assert.match(
    reasonOf(run(edit("order: 11", "order: 12"))),
    /src\/content\/laws\/basic-act-on-education\.md/
  );
});
...
test("長い URL でも差分の箇所が理由文に残る", () => {
  const base =
    "https://www.mext.go.jp/content/20260423-mxt_syoto01-000028144_0";
  const reason = reasonOf(
    run(edit(`    url: ${base}1.pdf`, `    url: ${base}2.pdf`))
  );
  ...
  assert.notEqual(
    before,
    after,
    "理由文の before と after が同一では確認しようがない"
  );
});
...
//
// **サイズと閾値は実測で決めた**(本機 Node 24、保護キー 7 本):
//   32KB / 線形       1.2ms
//   32KB / 全キー二次  12,091ms
//   32KB / 1 キーだけ二次  1,703ms   ← 部分的な退行もこの閾値で捕まる
//
// 当初は 256KB / 1000ms だった。**検知力は 32KB と変わらない**(1 キーだけの
// 退行も全キーの退行も、どちらのサイズでも閾値を超える)一方、赤になるまでの
// 待ち時間だけが伸びる: 256KB では 1 キーで 109 秒・全キーで約 12 分かかる。
// 32KB なら全キーでも 12 秒で、変異試験を回す速さが実用の範囲に収まる。
// 逆に 16KB まで縮めると 1 キーだけの退行が 425ms で閾値を割り、検知力が落ちる。
// 姉妹 2 リポ(edu-evidence / edu-watch)も同じ 32KB / 500ms で揃えてある。
...
test("ID だけの Edit でも発火する(ガイドページ)", () => {
  const reason = reasonOf(
    run(edit("323AC0000000120", "323AC0000000121", GUIDE))
  );
  assert.match(reason, /^  e-Gov law IDs:$/m);
});
...
test("ID パターンが通常の英数字列を拾わない", () => {
  const m = captureProtectedFields(
    "2026年 100VH #6b4423 ABC123 v1.2.3 20260423 SHA256HASH"
  );
...
test("ガイド: JS データの title / url は保護キーとして拾わない", () => {
  const before = [
    "  {",
    '    id: "rank-constitution-heading",',
    '    title: "憲法",',
    "  },",
  ].join("\n");
  const after = before.replace('"憲法"', '"日本国憲法"');
  assert.ok(
    !run(edit(before, after, GUIDE)).stdout,
    ".astro の JS データで鳴ると、ガイドを整えるたびに ask が出る"
  );

  const m = captureProtectedFields('    title: "憲法",', undefined, {
    keys: false,
  });
  assert.equal(m.get("title"), undefined);
});
...
test("diffMaps: 片側にしか無いキーも差分になる", () => {
  ...
  const after = new Map();
  assert.deepEqual(diffMaps(before, after), [
    { key: "order", before: ["1"], after: [] },
  ]);
});
...
const bigUrlList = (tag, count) =>
  Array.from(
    { length: count },
    (_, i) =>
      `https://www.mext.go.jp/content/2026${tag}-mxt_syoto01-000028${String(i).padStart(4, "0")}_01.pdf`
  ).join("\n");
...
test("settings.json: PreToolUse に Edit|Write|MultiEdit で登録されている", () => {
  const settings = JSON.parse(
    require("node:fs").readFileSync(SETTINGS, "utf8")
  );
  ...
  assert.ok(
    entry,
    'PreToolUse に matcher "Edit|Write|MultiEdit" のこのフックが見つからない'
  );
...
test("settings.json: branch-guard はここでは登録しない(global の dispatcher が持つ)", () => {
  ...
  assert.equal(
    found,
    false,
    "branch-guard.sh をリポ側で登録し直すと dispatcher 側の判定が効かなくなる"
  );
});
