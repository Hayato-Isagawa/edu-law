...
**現在の実体は見出しではなく太字**(`**直近の状態(YYYY-MM-DD …)**`)。上のコード例は 6 サブ節の並びを示すためのもので、マーカー行の書式は 3 通り(見出し 1 段 / 見出し 2 段 / 太字)に割れている。**「現在こうだから」で grep を決め打ちしないこと** — `archive/` には過去の書式がそのまま残っており、時期によって変わる。引くときは下の「アーカイブ手順」の union パターンを使う。
...
1. 切り出し位置を **union パターン**で確認する。
   ...
   **この履歴の中で時期によって変わっている**。マーカー語の後ろも一定ではない
   ...
   このリポでは 2026-08-28 に実際に踏んだ — 当時書かれていた
   `'^# 直近の状態\|^## 直近の状態'` が **0 件**を返したのに対し、実体は **13 ブロック**あった。
