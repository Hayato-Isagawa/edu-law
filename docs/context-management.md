...
**このリポの実体は見出しではなく太字**(`**直近の状態(YYYY-MM-DD …)**`)。上のコード例は 6 サブ節の並びを示すためのもので、マーカー行の書式はファミリー内で割れている(見出し 2 段 / 太字 / 見出し 1 段)。**このリポで grep するときは太字の形で引くこと**(下の「アーカイブ手順」も同じ)。
...
1. 切り出し位置を `grep -n '^\*\*直近の状態' .claude/state/active.md` で確認。**このリポのセッション開始行は見出しではなく太字**(`**直近の状態(YYYY-MM-DD …)**`)で、`#` を探す形は 1 件も返さない — 2026-08-28 の実測で、書いてあった `'^# 直近の状態\|^## 直近の状態'` が **0 件**に対し実体は **13 ブロック**あった。**0 件を「切り出す履歴が無い」と読まないこと**
...
- **2026-05-26 アーカイブ整理**: `archive/pre-2026-05-26-session-82cont3.md`(1,494 行)へ移動。期間はセッション #55 〜 #82続3(同ファイル冒頭の記載)。
- **2026-06-10 アーカイブ整理**: `archive/pre-2026-05-28-session-82cont15.md`(461 行)へ移動。**このファイルだけ冒頭の定型ヘッダが無い**ので、期間はファイル名(#82続15 まで)と mtime からの推定。
- **2026-08-28 アーカイブ整理**: 1,851 行 → 362 行(直近 9 ブロック=その16 〜 2026-08-26 その9 を残し、それ以前は `archive/pre-2026-08-28-its8.md`〈1,498 行〉へ移動)。

**この節は 3 回連続で更新が漏れていた**(2026-05-26 / 2026-06-10 の 2 回はアーカイブファイルだけが残り、記録が無かった)。**アーカイブしたら同じ作業の中でここに 1 行足すこと** — 後から書こうとすると、ファイル名と mtime からの推定しか残らない(上の 2026-06-10 の行がその状態)。
