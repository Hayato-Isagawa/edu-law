...
- 2026-09-17(#255): `@type` は `Organization` で終わるサブタイプと配列も `Organization` として拾い、集めた全 `Organization` の `url` のホストが自サイトであることを見る(許すキーだけで書いた姉妹組織のノードを別スロットに置く形を止める。ここまでは edu-watch #691 と同型)。加えて JSON-LD に現れる全ノードの `@type` をサイトが書く 6 種(`Article` / `BreadcrumbList` / `ListItem` / `Organization` / `Person` / `WebSite`)の閉じた集合に固定する — schema.org の `Organization` 下位クラスは 187(`schemaorg-current-https.jsonld` v30.1 を `rdfs:subClassOf` で推移的に辿った数)で、名前が `Organization` で終わるのは 8 つ。`Corporation` / `NGO` 等で書いた姉妹ノードは拾う側の網では抜けるため
- 2026-09-17(#257): JSON-LD の全ノードを型ごとのキー集合で検査し(`@type` の無いノード・`@id` だけの参照・集合外のキーで書いた関係は赤)、`sameAs` 以外の URL 値が自サイトを指すことと、トップレベルの `WebSite` が 1 本であることを見る。姉妹サイトを `WebSite` ノードや文字列値で書く形を止める(edu-watch #700 と同型)
- 2026-09-17(#259): `@context` は文字列 `https://schema.org` 1 形に固定しトップレベルにしか置かない。URL 値の判定は trim + 大文字小文字無視 + `//host` も拾う。`sameAs` は URL 文字列の配列に固定する。`Person`(著者)の `sameAs` にファミリーのドメインを置くのは許す(同一人物のページ。根拠は edu-watch ADR 0071 の 2026-09-17 #704 の項)
- 2026-09-18(#263): `sameAs` 以外の配列はオブジェクトの並び(`itemListElement` など)に限り、文字列の配列で URL を書く形を止める。http(s) スキームか `//` で始まる文字列は URL として構文解析してホストを見る(`https:host` など WHATWG だけがホストに解釈する形も拾う)。`sameAs` の要素は `https?://` に限る(edu-watch #709 と同型)
