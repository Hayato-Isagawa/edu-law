...
- 2026-09-17(#255): `@type` は `Organization` で終わるサブタイプと配列も `Organization` として拾い、集めた全 `Organization` の `url` のホストが自サイトであることを見る(許すキーだけで書いた姉妹組織のノードを別スロットに置く形を止める)。加えて JSON-LD に現れる全ノードの `@type` をサイトが書く 6 種(`Article` / `BreadcrumbList` / `ListItem` / `Organization` / `Person` / `WebSite`)の閉じた集合に固定する — schema.org の `Organization` 下位クラス 187 のうち名前が `Organization` で終わるのは 8 つで、`Corporation` / `NGO` 等で書いた姉妹ノードは拾う側の網では抜けるため(edu-watch #691 と同型)
