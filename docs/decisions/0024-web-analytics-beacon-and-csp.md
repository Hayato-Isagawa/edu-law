# 0024. Cloudflare Web Analytics を手動スニペット方式で導入し CSP を最小限緩和する(edu-evidence ADR 0026 ミラー)
...
1. law.edu-evidence.org 用の WA サイトを手動モードで**新規作成**し、Cloudflare Pages 統合の自動注入は無効化する
...
- デプロイ後に `POST https://cloudflareinsights.com/cdn-cgi/rum` の 204 を実測し、WA ダッシュボードへのイベント記録も確認(2026-07-12)
- 機構は evi / watch と同一。全ページに外部スクリプト 1 本(cookie なし・ドメイン単位)
